...
def get_args_parser():
    ...
    # * Finetuning params
    parser.add_argument('--finetune', default='models/checkpoints/pretrain_vit_b_1600.pth',
    ...
    parser.add_argument('--ffn_num', default=32, type=int, help='bottleneck middle dimension')
...
def main(args):
   ...
   #xxx#  dataset_train = build_training_dataset(args)
    #xxx# dataset_val, _ = build_dataset(is_train=False, test_mode=False, args=args)  #test.csv
    dataset_test, _ = build_dataset(is_train=False, test_mode=True, args=args)  #val.csv

    if True:  # args.distributed:
        num_tasks = misc.get_world_size()
        global_rank = misc.get_rank()
        #xxx# sampler_train = torch.utils.data.DistributedSampler(
        #xxx#     dataset_train, num_replicas=num_tasks, rank=global_rank, shuffle=True
        #xxx# )
       #xxx#  print("Sampler_train = %s" % str(sampler_train))
        #xxx# if args.dist_eval:
            #xxx# if len(dataset_val) % num_tasks != 0:
            #xxx#     print('Warning: Enabling distributed evaluation with an eval dataset not divisible by process number. '
            #xxx#           'This will slightly alter validation results as extra duplicate entries are added to achieve '
            #xxx#           'equal num of samples per-process.')
            #xxx# print("****")
            #xxx# sampler_val = torch.utils.data.DistributedSampler(
            #xxx#     dataset_val, num_replicas=num_tasks, rank=global_rank, shuffle=True)  # shuffle=True to reduce monitor bias
        sampler_test = torch.utils.data.DistributedSampler(dataset_test, num_replicas=num_tasks, rank=global_rank, shuffle=False)
        #xxx# else:
        #xxx#     sampler_val = torch.utils.data.SequentialSampler(dataset_val)
    ...
    #xxx# if args.finetune and not args.eval:
